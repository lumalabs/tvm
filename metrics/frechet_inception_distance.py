...
def compute_fid(opts, max_real, num_gen):
    ...
    if opts.ref_stats is None:
        mu_real, sigma_real = metric_utils.compute_feature_stats_for_dataset(
            opts=opts, detector_url=detector_url, detector_kwargs=detector_kwargs,
            rel_lo=0, rel_hi=0, capture_mean_cov=True, max_items=max_real).get_mean_cov()
    else:
        ref = load_stats(opts.ref_stats, verbose=False)
        mu_real, sigma_real = ref['fid']['mu'], ref['fid']['sigma']
    mu_gen, sigma_gen = metric_utils.compute_feature_stats_for_generator(
        opts=opts, detector_url=detector_url, detector_kwargs=detector_kwargs,
        rel_lo=0, rel_hi=1, capture_mean_cov=True, max_items=num_gen, batch_size=opts.batch_size, 
        batch_gen=opts.batch_size).get_mean_cov()
