...
class MetricOptions:
    def __init__(
        ...
        self.detector_kwargs = dnnlib.EasyDict(detector_kwargs)
...
class ProgressMonitor:
    ...
    def update(self, cur_items):
        ...
        if (self.progress_fn is not None) and (self.num_items is not None):
            self.progress_fn(
                self.pfn_lo
                + (self.pfn_hi - self.pfn_lo) * (cur_items / self.num_items),
                self.pfn_total,
            )
...
def compute_feature_stats_for_dataset(
    ...
    **stats_kwargs,
    ...
    progress = opts.progress.sub(
        tag="dataset features", num_items=num_items, rel_lo=rel_lo, rel_hi=rel_hi
    )
...
def compute_feature_stats_for_generator(
    ...
    **stats_kwargs,
    ...
    if jit:
        ...
        z = torch.zeros(
            [batch_gen, G.img_channels, G.img_resolution, G.img_resolution],
            device=opts.device,
        )
